...
public:
	 ...
	 */
    Cube build(int size){
		Face back = buildFace(size, "green");
		Face up = buildFace(size, "yellow");
		Face front = buildFace(size, "blue");
		Face left = buildFace(size, "orange");
		Face right = buildFace(size, "red");
		Face down = buildFace(size, "white");
